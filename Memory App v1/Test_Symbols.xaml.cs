...
namespace Memory_App_v1
{
    ...
    /// </summary>
    public sealed partial class Test_Symbols : Page
    {
        
       bool captionOn = false;

        int initialNumberOfNumbersToDisplay = 3;
        ...
        string[] possibleSymbols = new string[] { "!", "@", "#", "$", "%", "^", "&", "*", "?", "/" };
        ...
        public Test_Symbols()
        {
                ...
                highScores[0] = (int)applicationDataContainer.Values["HighScore1_Symbol"];
                highScores[1] = (int)applicationDataContainer.Values["HighScore2_Symbol"];
                highScores[2] = (int)applicationDataContainer.Values["HighScore3_Symbol"];
                highScores[3] = (int)applicationDataContainer.Values["HighScore4_Symbol"];
                highScores[4] = (int)applicationDataContainer.Values["HighScore5_Symbol"];
        ...
        void instructionsDispatcher_Tick(object sender, object e)
        {
            ...
            if (instructionDateTime.Second == 0)
            {
                ...
                tbkDisplaySymbols.FontSize = Frame.ActualHeight / 10;

                for (int i = 0; i < initialNumberOfNumbersToDisplay; i++)
                {

                    //we create Run objects because we can add individual colours to them
                    Run runText = new Run();
                    runText.Text = possibleSymbols[random.Next(0, 10)].ToString();

                    list_runs.Add(runText);

                    tbkDisplaySymbols.Inlines.Add(list_runs[i]);
                }
                ...
                dateTime = dateTime.AddSeconds(31);
        ...
        private void btnAnswer_Operation()
        {
            ...
            if (numberOfWrongAnswers == 2)
            {
                ...
                applicationDataContainer.Values["HighScore1_Symbol"] = highScores[0];
                applicationDataContainer.Values["HighScore2_Symbol"] = highScores[1];
                applicationDataContainer.Values["HighScore3_Symbol"] = highScores[2];
                applicationDataContainer.Values["HighScore4_Symbol"] = highScores[3];
                applicationDataContainer.Values["HighScore5_Symbol"] = highScores[4];



                tbkAnalysis.Opacity = 1;
                tbkAnalysis.Text = "";
                tbkAnalysis.Text = "The test is now finished. \n Maximum number of symbols memorized: " + (initialNumberOfNumbersToDisplay - 1).ToString(); ;
                btnAnswer.Visibility = Windows.UI.Xaml.Visibility.Collapsed;
                btnAnswerNow.Visibility = Windows.UI.Xaml.Visibility.Collapsed;

              

                //applicationDataContainer.Values.Remove("HighScore1_Symbol");
                //applicationDataContainer.Values.Remove("HighScore2_Symbol");
                //applicationDataContainer.Values.Remove("HighScore3_Symbol");
                //applicationDataContainer.Values.Remove("HighScore4_Symbol");
                //applicationDataContainer.Values.Remove("HighScore5_Symbol");
            }
            else
            {
        ...
        {
            if (dateTime.Second == 0 && dateTime.Minute == 0)       //when time for displaying the numbers in tbkDisplaySymbols is 0, we change its opacity to 0 and allow tbxAnswer to appear
            {
                dispatcherTimer.Stop();
                tbkDisplaySymbols.Opacity = 0;
        ...
        private void DisplayNumbers(int i)
        {
            ...
            tbkDisplaySymbols.Text = ""; //sets textblock to empty string here to make room for new numbers
            tbkDisplaySymbols.Opacity = 1;
            ...
            for (int a = 0; a < i; a++)
            {
                ...
                tbkDisplaySymbols.Inlines.Add(list_runs[a]);
            }


            dispatcherTimer.Tick += dispatcherTimerInitial_Tick;
            dateTime = dateTime.AddSeconds(i*10 + 1);
